...
- **Never guess** tools / `jf api` paths → tool list / `--help` / the `#jfrog-references` steering.
...
the `jf-api` section of the `#jfrog-references` steering,
the `preserving-command-output` section of the `#jfrog-references` steering,
the `cli-command-discovery` section of the `#jfrog-references` steering.
Setup / `jf setup` / ordinary CLI do **not** require Tier B.

**Tier C — on-demand:** the `INDEX` section of the `#jfrog-references` steering domain
...
| [When to read reference files](#when-to-read-reference-files) → the `INDEX` section of the `#jfrog-references` steering | Tier C domain refs |
...
> detail → Tier C the `INDEX` section of the `#jfrog-references` steering.
...
```bash
bash ~/.kiro/jfrog-scripts/jfrog/cli-newer-version-offer.sh
...
#   bash ~/.kiro/jfrog-scripts/jfrog/cli-newer-version-offer.sh --clear
# Yes → the `jfrog-cli-install-upgrade` section of the `#jfrog-references` steering, then check-environment.sh --force
...
skill and in the `#jfrog-references` steering omit the export for readability — the rule is
...
the `cli-gotchas` section of the `#jfrog-references` steering when you enter Tier B.

| Tier B — MUST read in full (path-gated) | Covers |
|-----------------------------------------|--------|
| the `cli-gotchas` section of the `#jfrog-references` steering | gotchas, caveats, known issues, do/don't, I/O & auth traps |
| the `jf-api` section of the `#jfrog-references` steering | product-prefix table, flags, examples, GraphQL payload |
| the `preserving-command-output` section of the `#jfrog-references` steering | temp files, `$$` paths, no re-fetch for `jq` |
| the `cli-command-discovery` section of the `#jfrog-references` steering | namespaces, top-level cmds, Pipelines sunset |
...
else under the `INDEX` section of the `#jfrog-references` steering is **Tier C** domain
detail — load ≤2–3 most specific files for the task; skip unused domains.

the `INDEX` section of the `#jfrog-references` steering lists every the `#jfrog-references` steering file (Tier B + Tier C).
...
More REST/template patterns → the `artifactory-api-gaps` section of the `#jfrog-references` steering or
the `platform-admin-api-gaps` section of the `#jfrog-references` steering via the `INDEX` section of the `#jfrog-references` steering.

## Gotchas — hard rules (never skip)

**Not tips.** Tier A bullets below are the always-read floor. Full
the `cli-gotchas` section of the `#jfrog-references` steering is **Tier B** —
...
- **Non-interactive only** — avoid `jf config add`, `jf login`, `*template`
  wizards; use the `jfrog-login-flow` section of the `#jfrog-references` steering / REST
...
the `general-parallel-execution` section of the `#jfrog-references` steering.
...
- [ ] never guess tools/paths → list / `--help` / the `#jfrog-references` steering; 404 → stop;
