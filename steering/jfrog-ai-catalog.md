...
| Intent | Read |
|--------|------|
| "What skills are available?" / browse the catalog / list versions / search by name | the `discovering-skills` section of the `#jfrog-ai-catalog-references` steering |
| Install or update a skill (latest or a pinned version), or a download is blocked | the `installing-skills` section of the `#jfrog-ai-catalog-references` steering |
| "What's installed?" / remove an installed skill | the `managing-installed-skills` section of the `#jfrog-ai-catalog-references` steering |
| Publish / upload / release a skill to the catalog | the `publishing-skills` section of the `#jfrog-ai-catalog-references` steering |
| "What plugins are available?" / browse the plugin catalog / list plugin versions / search plugins | the `discovering-plugins` section of the `#jfrog-ai-catalog-references` steering |
| Install or update a plugin (latest or a pinned version) | the `installing-plugins` section of the `#jfrog-ai-catalog-references` steering |
| "What plugins are installed?" / remove an installed plugin | the `managing-installed-plugins` section of the `#jfrog-ai-catalog-references` steering |
| Publish / upload / release a plugin to the catalog | the `publishing-plugins` section of the `#jfrog-ai-catalog-references` steering |
...
- **Read the base `jfrog` skill first.** the `#jfrog` steering
  ...
  - The [server selection rules](the `#jfrog` steering)
...
[`jfrog` skill](the `#jfrog` steering); follow those too. Flow-specific rules live in
...
> **References:** deep material for this skill ships as the `#jfrog-ai-catalog-references` steering (each former reference file is a `## <file>` section there). Load it on demand.
