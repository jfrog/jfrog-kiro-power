...
the `rca-procedure` section of the `#jfrog-package-curation-references` steering and follow it from
...
> **References:** deep material for this skill ships as the `#jfrog-package-curation-references` steering (each former reference file is a `## <file>` section there). Load it on demand.
