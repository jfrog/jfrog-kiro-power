...
`scripts/` / the `#jfrog-setup-package-managers-references` steering).

Apply the session hook's repo pick via [`jf setup`](the `jf-setup-command` section of the `#jfrog-setup-package-managers-references` steering),
then record it in [`.jfrog/local/package-resolution.json`](the `workspace-binding` section of the `#jfrog-setup-package-managers-references` steering)
via `~/.kiro/jfrog-scripts/jfrog-setup-package-managers/merge-workspace-binding.sh`.
...
- **Cover base the `#jfrog` steering At-a-glance / Tier A**
...
- **Never skip** [Gotchas](#gotchas--hard-rules-never-skip) + base Tier A hard
  rules (the `#jfrog` steering Cautious execution / Server selection / Tier A
...
`~/.kiro/jfrog-scripts/jfrog-setup-package-managers/merge-workspace-binding.sh`) when package-manager config is still missing.
...
the `#jfrog` steering (Cautious execution, Server selection,
...
- **Always pass `--repo` and `--server-id`** — omitting `--repo` fails when
  multiple repos match. See [`jf-setup-command.md`](the `jf-setup-command` section of the `#jfrog-setup-package-managers-references` steering).
...
- **Persist binding with the merge script** — after each successful `jf setup`,
  run `~/.kiro/jfrog-scripts/jfrog-setup-package-managers/merge-workspace-binding.sh` (Step 6). Do **not** hand-edit the JSON.
...
| File | When to read |
|------|--------------|
| the `jf-setup-command` section of the `#jfrog-setup-package-managers-references` steering | CLI flags, supported package managers, exit-code contract, `jf setup --help` |
...
| the `workspace-binding` section of the `#jfrog-setup-package-managers-references` steering | Workspace binding schema, package-manager → type map, merge script |
| `~/.kiro/jfrog-scripts/jfrog-setup-package-managers/merge-workspace-binding.sh` | After each successful `jf setup` — deterministic binding merge (`jq` required) |
...
1. **Cover base skill At-a-glance / Tier A before the first non-exempt `jf`
   ...
   the `#jfrog` steering in full when you can; the At-a-glance
...
2. **Workspace binding** — read `.jfrog/local/package-resolution.json`. Drop
   ...
   [`workspace-binding.md`](the `workspace-binding` section of the `#jfrog-setup-package-managers-references` steering).
3. **Workspace manifests** when still ambiguous (several package managers of one
   ...
   **Binary gate (client tools only):** missing client on `PATH` → skip as not
   ...
   [`jf-setup-command.md`](the `jf-setup-command` section of the `#jfrog-setup-package-managers-references` steering).

4. **`jf setup --help`** — filter candidates; never hardcode the list. See
   [`jf-setup-command.md`](the `jf-setup-command` section of the `#jfrog-setup-package-managers-references` steering). Unsupported → report
   gap, skip.
...
3. **Global cache** — last resort only; never overrides (1) or (2). See
   [`global-cache-file.md`](the `global-cache-file` section of the `#jfrog-setup-package-managers-references` steering).
...
> **References:** deep material for this skill ships as the `#jfrog-setup-package-managers-references` steering (each former reference file is a `## <file>` section there). Load it on demand.
> **Helper scripts** for this skill live at `~/.kiro/jfrog-scripts/jfrog-setup-package-managers/` once installed. They are not bundled with the Power — install them on demand with `npm run install-scripts` (or the equivalent fetch in POWER.md → Onboarding). If a script is missing (install skipped or offline), install it then retry, or perform the equivalent steps manually; the steering still drives everything else.
