...
description: "Use this skill when the user asks to set up, configure, bind, or connect a package manager (npm, pip, maven, gradle, go, docker, helm, …) to JFrog Artifactory via `jf setup` and `.jfrog/local/package-resolution.json`; when a workspace manifest exists with no matching binding entry; or when a session hook reports PM config missing. Skip when the binding already has the same repo key — the session hook reapplies each start. Never pick a repo by discovery; use resolver output only (unless the user explicitly names or asks to browse repos). On unresolved or failed setup, ask for a repo key with the failure verbatim — never switch servers."
---
...
2. **Workspace binding** — read `.jfrog/local/package-resolution.json`. Drop PMs
   ...
   key). PM → type table: [`workspace-binding.md`](the `workspace-binding` section of the `#jfrog-setup-package-managers-references` steering).
...
4. **`jf setup --help`** — filter candidates; never hardcode the PM list. See
   [`jf-setup-command.md`](the `jf-setup-command` section of the `#jfrog-setup-package-managers-references` steering). Unsupported PM →
   report gap, skip.
