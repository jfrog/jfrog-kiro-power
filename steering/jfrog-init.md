...
- Everything else is read-only, except: Step 5's placeholder
  ...
  `AskUserQuestion` — see the `script-invocation` section of the `#jfrog-init-references` steering); Step 8's
  `~/.netrc` write (same, see the `marketplace-setup` section of the `#jfrog-init-references` steering); and
...
the `script-invocation` section of the `#jfrog-init-references` steering.

## Step 0: resolve the `${CLAUDE_SKILL_DIR}` placeholder (do this FIRST)

**`${CLAUDE_SKILL_DIR}` is this skill's own absolute directory** — the
folder holding this `SKILL.md`, `scripts/`, and the `#jfrog-init-references` steering. Every
...
- **Every other harness** — you MUST look it up and export it before
  ...
  1. Look up this skill's absolute directory from wherever your
     ...
     the `mcp-plugin-config` section of the `#jfrog-init-references` steering). On OpenCode specifically, it
...
the `script-invocation` section of the `#jfrog-init-references` steering for why. Step 0 resolves *script
...
- **Capturing the exit code before forcing success is mandatory** (`;
  ...
  the `invoking-and-output-rules` section of the `#jfrog-init-references` steering's "Invoking scripts" section). A
...
- **Never surface the checklist.** Run silently — no step narration, no
  ...
  the `invoking-and-output-rules` section of the `#jfrog-init-references` steering's "Customer-facing output" section.
...
- **This skill is the exception to, not a consumer of, the base
  the `#jfrog` steering's prerequisites** — do not
...
- **Read the base the `#jfrog` steering for foundational
...
- **This skill is the exception that makes the base skill's prerequisites
  ...
  [environment check](the `#jfrog` steering) assumes `jf` is
...
- **Deliberately does not export `JFROG_CLI_USER_AGENT`**, unlike the
  base skill's invariant — see `runJf()` in `~/.kiro/jfrog-scripts/jfrog-init/lib/jf.mjs` for why
  (telemetry-only impact).

## Operating rules

**Stop and read the `invoking-and-output-rules` section of the `#jfrog-init-references` steering in full before running
...
- **Flow** — follow the flow literally, no reordering or skipping; see
  the `flow-diagram` section of the `#jfrog-init-references` steering for the full map.

## The checklist, in order

1. **Node.js ≥ 18 installed?** — no script; run `node --version` / `npx --version` directly
2. **JFrog CLI (`jf`) installed?** — `~/.kiro/jfrog-scripts/jfrog-init/jfrog-detect-jf-cli.mjs`
3. **`jf` connected to a server?** — `~/.kiro/jfrog-scripts/jfrog-init/jfrog-detect-jf-config.mjs`
4. **Server reachable + credentials valid?** — `~/.kiro/jfrog-scripts/jfrog-init/jfrog-detect-server-ping.mjs [server-id]`
5. **JFrog MCP plugin file has a jfrog entry?** — `~/.kiro/jfrog-scripts/jfrog-init/jfrog-detect-jfrog-mcp.mjs [server-id]`, then **is the JFrog MCP server enabled on this JPD?** — `~/.kiro/jfrog-scripts/jfrog-init/jfrog-detect-jfrog-mcp-responding.mjs [server-id]` (non-blocking)
   - **5b (OpenCode only): MCP auth token present?** —
     `~/.kiro/jfrog-scripts/jfrog-init/jfrog-detect-opencode-mcp-auth.mjs`, runs only when Step 5
     exits 0 and the harness is OpenCode
6. **Project resolved?** — `~/.kiro/jfrog-scripts/jfrog-init/jfrog-detect-project.mjs [server-id] [project-input]`
7. **AI Catalog reachable & user entitled?** — `~/.kiro/jfrog-scripts/jfrog-init/jfrog-detect-catalog-runtime.mjs [server-id]`
8. **Claude agent-plugin marketplace registered?** — `~/.kiro/jfrog-scripts/jfrog-init/jfrog-add-claude-marketplace.mjs [server-id] [project-key]`, Claude Code only
...
through the single shared `~/.kiro/jfrog-scripts/jfrog-init/jfrog-resolve-jf-server.mjs`):
...
3. **The server flagged `"isDefault": true`** in `~/.jfrog/jfrog-cli.conf.v6`
   — resolved automatically via `~/.kiro/jfrog-scripts/jfrog-init/jfrog-resolve-jf-server.mjs`.
...
5. Otherwise the detector exits **2 ("ask")** with a JSON `candidates`
   ...
   the `server-picker` section of the `#jfrog-init-references` steering in full** — it has the exact
...
On failure, **stop and read the `node-install-prompt` section of the `#jfrog-init-references` steering in full
...
- **Exit 1, `reason: "missing"`** → `jf` not found on PATH → **stop
  and read the `jf-cli-install-prompt` section of the `#jfrog-init-references` steering in full** — required
  behavior, not optional background.
- **Exit 1, `reason: "broken"`** → `jf` is on PATH but hung or
  failed to run → **stop and read the `jf-cli-install-prompt` section of the `#jfrog-init-references` steering
...
- **Exit 1, `reason: "outdated"`** → `jf` installed but below the
  ...
  the `jf-cli-update-prompt` section of the `#jfrog-init-references` steering in full** — required behavior,
  not optional background.

**Read the `jf-cli-install-internals` section of the `#jfrog-init-references` steering for the full plan
...
- **Exit 1** → `jf` is installed but not connected to any
  server. **Stop and read the `jf-config-auth-picker` section of the `#jfrog-init-references` steering in
...
**Stop and read the `jf-config-auth-picker` section of the `#jfrog-init-references` steering's "Step 4's full
...
the `mcp-plugin-config` section of the `#jfrog-init-references` steering).

**Stop and read the `mcp-plugin-config` section of the `#jfrog-init-references` steering in full** — substitution mechanics,
...
**Stop and read the `mcp-plugin-config` section of the `#jfrog-init-references` steering in full** for what
...
**stop and read the `project-state-reuse` section of the `#jfrog-init-references` steering in full** — it has
...
fuzzier tiers — see the `project-matching` section of the `#jfrog-init-references` steering for the algorithm.
...
**stop and read the `project-picker` section of the `#jfrog-init-references` steering in full before
...
**Stop and read the `project-resolution-branches` section of the `#jfrog-init-references` steering in full**
...
**Stop and read the `catalog-runtime-branches` section of the `#jfrog-init-references` steering in full** —
...
`~/.kiro/jfrog-scripts/jfrog-init/jfrog-resolve-mcp-config.mjs` (the same export Step 5b already
uses), e.g.
`node "${CLAUDE_SKILL_DIR}/scripts/jfrog-resolve-mcp-config.mjs" --harness`.
...
**Stop and read the `marketplace-setup` section of the `#jfrog-init-references` steering in full before
...
**Read the `final-summary-rendering` section of the `#jfrog-init-references` steering in full** for the exact
recap checklist format, the ⚠️ wording rule per outstanding step, and
Step 8's extra checklist line.

## Running everything at once

**Read the `batch-walk` section of the `#jfrog-init-references` steering in full** for `jfrog-detect-all.mjs`'s
...
**Read the `out-of-scope` section of the `#jfrog-init-references` steering in full** for the exact list — IDE
...
- [ ] Never store, log, or print an access token — except Step 8's
      `~/.netrc` write
- [ ] Read the base the `#jfrog` steering for context; do not run its
      environment check as a gate before this walk

> **References:** deep material for this skill ships as the `#jfrog-init-references` steering (each former reference file is a `## <file>` section there). Load it on demand.
> **Helper scripts** for this skill live at `~/.kiro/jfrog-scripts/jfrog-init/` once installed. They are not bundled with the Power — install them on demand with `npm run install-scripts` (or the equivalent fetch in POWER.md → Onboarding). If a script is missing (install skipped or offline), install it then retry, or perform the equivalent steps manually; the steering still drives everything else.
