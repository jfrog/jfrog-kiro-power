...
- Everything else in the walk is read-only, except Step 5's placeholder
  ...
  no `AskUserQuestion`, see the `script-invocation` section of the `#jfrog-init-references` steering), Step 8's
  `~/.netrc` write (also unattended, no `AskUserQuestion` — see
  the `marketplace-setup` section of the `#jfrog-init-references` steering), and the Final summary's state
  write below.
...
regardless of install depth (see the `script-invocation` section of the `#jfrog-init-references` steering). On
...
5. **JFrog MCP plugin file has a jfrog entry?** — `~/.kiro/jfrog-scripts/jfrog-init/jfrog-detect-jfrog-mcp.mjs [server-id]`
...
On red, **stop and read the `node-install-prompt` section of the `#jfrog-init-references` steering in full
...
**Stop and read the `mcp-plugin-config` section of the `#jfrog-init-references` steering in full** — exactly
...
`~/.kiro/jfrog-scripts/jfrog-init/jfrog-resolve-mcp-config.mjs` (the same export Step 5 already
uses), e.g.
