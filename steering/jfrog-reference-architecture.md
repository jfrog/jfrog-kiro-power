...
- Read the `#jfrog` steering for JFrog Platform concepts, product vocabulary, and routing to other workflows.
...
3. Note approximate response size. If over **1 MB**, or truncated, follow the downgrade path in the `doc-access` section of the `#jfrog-reference-architecture-references` steering (sitemap + targeted `index.md`).
...
> **References:** deep material for this skill ships as the `#jfrog-reference-architecture-references` steering (each former reference file is a `## <file>` section there). Load it on demand.
