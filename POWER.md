...
2. **(On demand) Install the JFrog helper scripts.** The steering already provides the full JFrog
   ...
   from the **same pinned version** (`v0.34.0`) and land in `~/.kiro/jfrog-scripts/<skill>/`; this is the
   ...
   *macOS / Linux:*
   ...
   ```bash
   TMP="$(mktemp -d)"
   curl -fsSL https://codeload.github.com/jfrog/jfrog-skills/tar.gz/v0.34.0 | tar -xz -C "$TMP"
   ...
   *Windows (PowerShell — `.zip` + built-in `Expand-Archive`, no `tar` needed):*
   ...
   Invoke-WebRequest https://codeload.github.com/jfrog/jfrog-skills/zip/v0.34.0 -OutFile "$tmp\s.zip"
