...
2. **(On demand) Install the JFrog helper scripts.** The steering already provides the full JFrog
   ...
   from the **bundled skills tree** in this repo (see [`scripts/sync-skills-vendor.json`](scripts/sync-skills-vendor.json)) and land in `~/.kiro/jfrog-scripts/<skill>/`; this is the
   ...
   *macOS / Linux:*
   ...
   ```bash
   TMP="$(mktemp -d)"
   curl -fsSL https://codeload.github.com/jfrog/jfrog-skills/tar.gz/refs/heads/main | tar -xz -C "$TMP"
   ...
   *Windows (PowerShell — `.zip` + built-in `Expand-Archive`, no `tar` needed):*
   ...
   Invoke-WebRequest https://codeload.github.com/jfrog/jfrog-skills/zip/refs/heads/main -OutFile "$tmp\s.zip"
